...
from config_attr import Common, Train
from model_attr import model as weatherModel
from data_loader_attr import trainLoader, valLoader
...
if __name__ == '__main__':
    ...
    #         print(valAcc, "保存模型中...")
    #         torch.save(model,
    #                    Train.modelDir + "clothing_reg-" + time.strftime('%Y-%m-%d-%H-%M-%S', time.gmtime()) + ".pth")
    # # 保存模型
    torch.save(model, Train.modelDir + "clothing_reg-attr" + time.strftime('%Y-%m-%d-%H-%M-%S', time.gmtime()) + ".pth")
