...
def loadDataFromDir():
    ...
    labels = []
    # 1. 获取根文件夹下所有分类文件夹
    for imagePath in os.listdir(Common.basePath):  # 2. 获取某一类型下所有的图片名称
        if not imagePath.endswith("jpeg"):
            continue
        # 3. 读取文件
        image = Image.open(Common.basePath + imagePath).convert('RGB')
        if len(images) % 100 == 0:
        ...
        # 5. 构造label
        label = [0] * Common.label_num  # 初始化label
        label_names = imagePath.split(',')[-2].split('=')[1].split("|")
        for label_i in label_names:
            categoryIndex = Common.labels.index(label_i)  # 获取分类下标
